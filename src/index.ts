...
async function checkGameBuild(gamePath: string): Promise<InstallationType> {
    ...
    if (await doesDirectoryExist(path.join(gamePath, MANAGED_DLL_DIR))) {
        ...
        const monoDlls = (await fs.readdirAsync(path.join(gamePath, MANAGED_DLL_DIR))).filter((f: string) => f.startsWith('Mono.') && f.endsWith('.dll'));
    ...
    throw new Error(`Couldn't find "${MANAGED_DLL_DIR}" or "${IL2CPP_DIR}" under the game folder, is the game installed correctly?`);
}

async function checkBepInExBuild(gamePath: string): Promise<InstallationType | undefined> {
    ...
    if (await doesDirectoryExist(bepInExPath)) {
        ...
        if (coreDlls.some((file: string) => file.toLowerCase().includes('il2cpp') && file.endsWith('.dll'))) {
    ...
    context.once(async () => {
        if (context.api.ext.bepinexAddGame !== undefined) {
            ...
            if (bepInExBuild && bepInExBuild !== gameBuild) {
                console.warn(`Detected BepInEx build mismatch: expected ${gameBuild}, but got ${bepInExBuild}.`);
                ...
                context.api.showDialog('error', 'BepInEx Version Mismatch', {
                    bbcode: t('"{{game}}" is currently installed with the "{{gameBuild}}" game build,{{bl}}' +
                        'but the currently installed BepInEx version is for the {{bepInExBuild}} build.{{bl}}' +
                        'Please go to the Mods tab and install the correct BepInEx version for your game build.', { replace }),
                }, [
